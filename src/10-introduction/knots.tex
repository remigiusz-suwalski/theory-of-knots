...
Zanim wykluczymy je z~naszych rozważań, zastanówmy się, jakim formalizmem opisać manipulowanie fizycznym sznurkiem.
Nie można użyć izotopii (dwa węzły są izotopijne, jeśli istnieje ciągła funkcja $F \colon S^1 \times [0, 1] \to \R^3$ taka, że $F(-, 0)$ jest pierwszym, zaś $F(-,1)$ drugim węzłem), gdyż każdy supeł ściąga się do punktu:
...
Oznacza to, że zwykła izotopia nie oddaje dobrze tego, czym jest równoważność węzłów wykonanych z~prawdziwego sznurka.
...
Intuicyjnie oznacza to, że dwa węzły uznajemy za jednakowe, jeśli można umieścić jeden z nich w~przestrzeni $\R^3$, zdeformować ją i~otrzymać drugi węzeł.
...
Homeomorfizmy $F_t$ można zastąpić przez dyfeomorfizmy zachowujące orientację.
% https://math.stackexchange.com/questions/1311865/equivalence-of-knots-ambient-isotopy-vs-homeomorphism/1312016#1312016
% https://mathoverflow.net/questions/185272/reference-for-a-fact-on-homeomorphic-knot-complements
...
Formalnie węzły to pewne odwzorowania, więc prawidłowym sposobem na zapisanie, że są równe, jest $K_1 \simeq K_2$.
Dość często będziemy jednak utożsamiać węzeł z~jego obrazem, dlatego w~książce dominuje zapis $K_1 = K_2$.
...
Wcześniej pokazano, że istnieją co najwyżej dwa węzły o~zadanym dopełnieniu (\cite{culler87}).
...
Odpowiednik twierdzenia \ref{thm_gordon_luecke} dla splotów jest fałszywy.
Wiedział o~tym już Whitehead w~1937 roku.
Jego praca \cite{whitehead37} zawiera nieskończenie wiele splotów, których dopełnienia wyglądają jak dopełnienia splotu Whiteheada.
