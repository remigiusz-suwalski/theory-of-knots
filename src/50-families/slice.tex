...
,,Bycie zgodnym'' jest relacją równoważności.
\textbf{Zgodność jest relacją równoważności, słabszą od izotopii, ale mocniejszą od homotopii.}
Klasę abstrakcji węzła $K$ oznaczamy przez $[K]$.
Iloraz zbioru węzłów przez tę relację, $C^1$, wyposażony w działanie
\begin{equation}
    [K_1] + [K_2] = [K_1 \shrap K_2]
\end{equation}
staje się grupą abelową, której elementem neutralnym jest klasa abstrakcji niewęzła, zaś elementem przeciwnym do $[K]$ jest $[mr K]$.
\index{grupa!zgodności}
Nazywamy ją grupą zgodności.
...
Więcej informacji w~podręczniku Kawauchiego \cite[s. 154-169]{kawauchi96}.
