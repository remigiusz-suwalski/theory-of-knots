...
\begin{proposition}
    ...
    Wtedy $\Delta(x)$, jego wielomian Alexandera, jest kwadratem w~pierścieniu $\Z_2[x]$.
\end{proposition}
...
\begin{proof}
    ...
    Wtedy wielomian $\Delta(x)$ jest kwadratem w pierścieniu $\Z[x]$ (pokazano to w \cite{hartley79}).
    ...
    \begin{equation}
        ...
        \Delta_K(t) \equiv \Delta^2_{J}(t) \mod 2,
    \end{equation}
    która kończy dowód.
\end{proof}
...
Wielomian $\Delta(t) = A_0 + A_1(t+1/t) + \ldots + A_n(t^n + 1/t^n)$ jest kwadratem modulo $2$ wtedy i tylko wtedy, gdy współczynniki $A_{2k+1}$ są parzyste dla $k \ge 0$.
...
\begin{proof}
    Niech $\Delta(t) = A_0 + A_1(t+1/t) + \ldots + A_n(t^n + 1/t^n)$ będzie  wielomianem Alexandera.
    Wtedy $\Delta_K(1) = \pm 1$, zatem $0 = \pm 1 + A_0 + 2A_1 + \ldots + 2A_n$.
    Możemy teraz odjąć to od $\Delta_K(-1) = A_0 - 2A_1 \pm \ldots$, by uzyskać $\Delta_K(-1) = \pm 1 - 4A_1 - 4A_3 - \ldots$.
    Razem z wygodnym przeformułowaniem daje to $\Delta_K(-1) \equiv \pm 1 \mod 8$, co w połączeniu z~\ref{arf_murasugi} kończy dowód.
\end{proof}
...
\begin{proof}
    Wynika to z~naszego wygodnego przeformułowania oraz stwierdzenia 8.16 w książce \cite{burde03}: jeśli $K$ jest włóknistym węzłem o genusie $g$, to wielomian Alexandera $\Delta_K$ jest stopnia $2g$ i~ma wiodący współczynnik $\pm 1$.
\end{proof}

\begin{corollary}
	\label{lissajous_twobridge}
    Jeśli $K$ jest dwumostowym wezłem Lissajous, to $\Delta_K(t) \equiv 1 \mod 2$.
\end{corollary}
...
\begin{proof}
    ...
    Wtedy jego faktor jest trywialny i~kongruencja \ref{lissajous_square_equiv} daje $\Delta_K(t) \equiv 1 \mod 2$.
\end{proof}
...
Obiekt ten, $(p, q)$-kabel, zdefiniowano w książce Eisenbuda/Neumanna ,,Three-dimensional link theory and invariants of plane curve singularities'', celowo pominiętej w bibliografii.

\begin{proof}
	Niech $L$ będzie $(p, q)$-kablem węzła $K$.
	Seifert pokazał w 1950 (praca \cite{seifert50}), że
    \begin{equation}
    	\Delta_L(t) = \frac{(t^{pq}-1)(t-1)}{(t^p-1)(t^q-1)} \Delta_K(t^p).
    \end{equation}
    Dwa najwyższe współczynniki w wielomianie ułamkowym to $\pm 1$, a skoro $|p| > 1$, dwa najwyższe współczynniki $\Delta_L$ to także $\pm 1$.
